...
from google_review_collector import searchNearbyPlaces
...
def get_location(myCursor,location_table_name,status,category):
    if status == "not_start":
        query = f"select * from {location_table_name} where state = 'New York' and location is not null and location !='None' and location_flag_start is null and location_flag_stop is null order by rand() limit 1"
    elif status == "start":
        query = f"select * from {location_table_name} where state = 'New York' and location is not null and location !='None' and location_flag_start = 1 and location_flag_stop is null order by rand() limit 1"
    elif status == "stop":
        query = f"select * from {location_table_name} where state = 'New York' and location is not null and location !='None' and location_flag_start = 1 and location_flag_stop = 1 order by rand() limit 1"
...
# def get_specific_location_from_db():
#     query = 
...
#<----------static variable declearation------------->
    ...
    location_table = f'{category}_locations'
    myDatabase, myCursor = dbConnection()
    gmbDataTableName = f'{category}_data'
    location_in_progress_table = 'location_in_progress'
    data_file_name = 'data.csv'
    instance_id = 2

    is_table_exist_and_create(myDatabase= myDatabase, myCursor= myCursor, category= category)
    time.sleep(3)
